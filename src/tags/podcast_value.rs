...
use crate::parser_state::{ParserState, PodcastValue, PodcastValueModel, PodcastValueRecipient};
...
pub fn on_value_recipient(attributes: &[OwnedAttribute], state: &mut ParserState) {
    ...
    for attr in attributes {
        match attr.name.local_name.as_str() {
            ...
            "fee" => vr.fee = matches!(attr.value.to_ascii_lowercase().as_str(), "true" | "yes" | "1"),
