...
pub fn on_start(current_element: &str, attributes: &[OwnedAttribute], state: &mut ParserState) {
    if (current_element != "podcast:locked" && current_element != "locked") || !state.in_channel || state.in_item {
        return;
    }
...
pub fn on_text(data: &str, state: &mut ParserState) {
    if state.in_channel_podcast_locked {
        ...
        if matches!(val.as_str(), "yes" | "true" | "1") {
...
pub fn on_end(state: &mut ParserState) {
    ...
    if state.channel_podcast_owner.trim().is_empty() && state.channel_podcast_locked == 1 {
        state.channel_podcast_owner = state.channel_itunes_owner_email.clone();
    }
}
